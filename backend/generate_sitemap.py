...
def generate_sitemap():
    ...
    if len(card_ids) > 50000:
        ...
        generate_sitemap_index(card_ids)
    else:
        generate_single_sitemap(card_ids)

def generate_single_sitemap(card_ids):
    ...
    print(f"総URL数: {len(static_pages) + len(card_ids)}")

def generate_sitemap_index(card_ids):
    today = datetime.now().strftime("%Y-%m-%d")
